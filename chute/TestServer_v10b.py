...
if (__name__ == "__main__"):
    ...
    startInst=datetime.now()
    f = open('debug.txt','a+') #server side processing of debug info
    subnet = ""
    ip = ''

    clientConn=start_server(ip)

    CP = 4 #initial CP we are looking for
    CP_last = 8 #last CP we need
    ...
    while (True):
        try:
		    ...
		    f.write("recieve time: {}ms,  processing time: {}ms \n".format(g1,g2))
